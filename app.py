...
# -----------------------------
# Bar chart for selected pollutants
# -----------------------------
if not filtered_df.empty and pollutants:
    st.subheader(f"📊 Selected Pollutant Levels in {st.session_state.city_selected}")
    fig = px.bar(
        filtered_df.melt(id_vars=["city"], value_vars=pollutants, var_name="Pollutant", value_name="Level"),
        x="Pollutant", y="Level", color="Pollutant",
        title=f"Pollutant Levels in {st.session_state.city_selected}", text="Level"
    )
